...
class CollisionAvoidanceLimit(Limit):
    ...
    def compute_qp_inequalities(
        ...
        # print("Computing collision avoidance limits...")
        for idx, (geom1_id, geom2_id) in enumerate(self.geom_id_pairs):
            ...
            if contact.inactive:
                # print(f"[INACTIVE] {geom1_name} ({geom1_id}) vs {geom2_name} ({geom2_id}) — dist = {contact.dist:.5f}")
                continue
            # else:
            #     print(f"[ACTIVE] {geom1_name} ({geom1_id}) vs {geom2_name} ({geom2_id}) — dist = {contact.dist:.5f}")
            ...
            if hi_bound_dist > self.minimum_distance_from_collisions:
                ...
                #CHANGED TO NOT ALLOW FULL COLLISION (POSITION WISE)
                dist = hi_bound_dist - self.minimum_distance_from_collisions
                upper_bound[idx] = (dist / dt) + self.bound_relaxation
