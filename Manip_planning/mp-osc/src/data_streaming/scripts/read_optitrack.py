...
class DigitalTwin:
    def __init__(self):
        ...
        self.cylinder_offset = np.array([0, -0.5, 0])
        self.cylinder_subs = []
        self.cylinder_poses = []
        self.cylinder_bases = []
        for i in range(8):
        ...
        self.joint_state_sub = rospy.Subscriber('/joint_states', JointState, self.joint_state_sub_cb)
        self.curr_joint_state = [0] * 7
    ...
    def joint_state_sub_cb(self, msg):
        self.curr_joint_state = np.array(msg.position[1:])
        self.curr_joint_state = np.degrees(self.curr_joint_state)

    def env_base_sub_cb(self, msg):
        pose = msg.pose.position
        self.env_base_pose[0] = -pose.y
        self.env_base_pose[1] = pose.z + 0.07
    ...
    def cyl0_sub(self, msg):
        pose = msg.pose.position
        self.cylinder_poses[0][0] = -0.035 - pose.y
        self.cylinder_poses[0][1] = pose.z
        self.cylinder_poses[0][2] = pose.x - 0.01
    ...
    def step(self):
        ...
        self.env.robot.setJointPositionsDirectly(self.curr_joint_state)
