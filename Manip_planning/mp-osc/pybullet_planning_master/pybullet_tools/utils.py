...
def set_camera_pose(camera_point, target_point):
...
def create_box(boxPosition,box_dims):    
    ...
    temp_col_box_id = p.createCollisionShape(p.GEOM_BOX, halfExtents=[boxHalfLength, boxHalfWidth, boxHalfHeight])

    # Create the multi-body with the box collision shape and set mass to 0 to make it fixed
    col_box_id = p.createMultiBody(baseMass=0, baseCollisionShapeIndex=temp_col_box_id, basePosition=boxPosition)
    return col_box_id


def create_cylinder(radius, height, position, quat, mass=STATIC_MASS, color=BLUE, **kwargs):
